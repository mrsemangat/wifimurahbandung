...
export default function CountdownTimer() {
  ...
  useEffect(() => {
    fetch('/api/settings')
      .then((r) => r.json())
      .then((data) => {
        if (data?.countdownEnabled === 'true' || data?.countdownEnabled === true) {
          setSettings({
            enabled: true,
            text: data.countdownText || defaultSettings.text,
          })
        } else if (data && typeof data === 'object' && !('error' in data)) {
          // Settings exist but no countdown config — use default
